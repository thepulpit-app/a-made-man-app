...
function MediaFrame({ item, tall = false }: { item: Resource; tall?: boolean }) {
    ...
    <div className="overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950">
      {embedUrl ? (
        <div className={tall ? 'aspect-[9/16] w-full' : 'aspect-video w-full'}>
          <iframe src={embedUrl} title={item.title} className="h-full w-full"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
            allowFullScreen />
        </div>
      ) : item.thumbnail_url ? (
        <img src={item.thumbnail_url} alt={item.title}
          className={tall ? 'aspect-[9/16] w-full object-cover' : 'h-56 w-full object-cover'} />
      ) : null}
      <div className="p-5">
        <p className="text-xs uppercase tracking-[0.25em] text-zinc-500">{item.type}</p>
...
export default function HomePage() {
  ...
  const conversation = resources.find((item) => item.display_section === 'conversation')
    ...
    <main className="min-h-screen bg-black text-white">
      <section className="px-6 pb-14 pt-8">
        <div className="mx-auto max-w-md space-y-7">
          <div className="flex items-center gap-4">
            <img src="/branding/made-logo.png" alt="A MADE MAN" className="h-14 w-auto" />
            <div>
              <p className="text-sm uppercase tracking-[0.45em] text-zinc-400">A MADE MAN</p>
              <p className="text-xs text-zinc-500">MEN. ADVOCACY. DIRECTION. EXCELLENCE.</p>
            </div>
          </div>

          {hero && (
            <div className="overflow-hidden rounded-[2rem] border border-zinc-800 bg-zinc-950 shadow-2xl">
              <img src="/branding/made-man-hero.png" alt="I Am A Made Man"
                className="aspect-video w-full object-cover object-center" />
            </div>
          )}

          <div>
            <p className="text-sm uppercase tracking-[0.35em] text-zinc-500">The Movement</p>
            ...
            {hero && (
              <div className="mt-6 border-l-2 border-zinc-700 pl-4">
                <p className="text-xs uppercase tracking-[0.25em] text-zinc-500">Featured</p>
                <p className="mt-2 text-lg font-semibold">{hero.title}</p>
                {hero.description && <p className="mt-2 text-sm text-zinc-400">{hero.description}</p>}
              </div>
            )}

            {/* FIX: Enter App goes to /dashboard, not /login.
                The dashboard's own auth check handles unauthenticated users.
                Logged-in users go straight in. Logged-out users get sent to /login from there. */}
      ...
      <section className="border-t border-zinc-900 px-6 py-16">
        <div className="mx-auto max-w-md">
          <p className="text-sm uppercase tracking-[0.4em] text-zinc-500">The Movement</p>
          <h2 className="mt-4 text-4xl font-bold">Conversations that build men.</h2>
          <p className="mt-6 text-zinc-400">
            Conference sessions. Brotherhood conversations. Podcast episodes.
            Real principles for real men navigating purpose, pressure, leadership, faith, family, and legacy.
          </p>
        </div>
      </section>
      ...
      {conversation && (
        <section className="border-t border-zinc-900 px-6 py-16">
          <div className="mx-auto max-w-md space-y-8">
            <div>
              <p className="text-sm uppercase tracking-[0.4em] text-zinc-500">Conversations</p>
              <h2 className="mt-4 text-4xl font-bold">Featured Conversation</h2>
              <p className="mt-4 text-zinc-400">
                Honest conversations about leadership, discipline, masculinity, family, pressure, purpose, and legacy.
              </p>
            </div>
      ...
      {shorts.length > 0 && (
        <section className="border-t border-zinc-900 px-6 py-16">
          <div className="mx-auto max-w-md space-y-6">
            <div>
              <p className="text-sm uppercase tracking-[0.4em] text-zinc-500">Shorts</p>
      ...
      {replays.length > 0 && (
        <section className="border-t border-zinc-900 px-6 py-16">
          <div className="mx-auto max-w-md space-y-6">
            <div>
              <p className="text-sm uppercase tracking-[0.4em] text-zinc-500">Conference</p>
              <h2 className="mt-4 text-4xl font-bold">Conference Replays</h2>
            </div>
            <div className="space-y-6">
              {replays.slice(0, 3).map((item) => <MediaFrame key={item.id} item={item} />)}
            </div>
          </div>
        </section>
      )}
      ...
      {podcasts.length > 0 && (
        <section className="border-t border-zinc-900 px-6 py-16">
          <div className="mx-auto max-w-md space-y-6">
            <div>
              <p className="text-sm uppercase tracking-[0.4em] text-zinc-500">Podcast</p>
              <h2 className="mt-4 text-4xl font-bold">Studio Episodes</h2>
            </div>
            <div className="space-y-6">
              {podcasts.slice(0, 3).map((item) => <MediaFrame key={item.id} item={item} />)}
            </div>
          </div>
        </section>
      )}
